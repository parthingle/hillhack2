...
string findSenators(senator sArr[], int size, string article);
...
int main()
{
	...
	//}
	cout << findSenators(sArray, 6, test);
}

string findSenators(senator sArr[], int size, string article) // |
{
	string result;
	for (int i = 0; i < size; i++) // syntax to loop through an array
	{
		int ind = indexInText(article, sArr[i].getName()); // find person if they're there
		result += to_string(ind);
		result += ' ';
		if (ind != -1) // find locations of last name
		{
			string lastName = sArr[i].getName().substr(sArr[i].getName().find(' '), sArr[i].getName().size()); // last name of current politician
			for (int j = i + sArr[i].getName().size(); j < article.size(); j++)
			{
				if (article.substr(j, lastName.size()) == lastName)
				{
					result += to_string(j);
					result += ' ';
				}
